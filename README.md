...
- Nice. Absolutely nothing shows up except the `Accounts` collection. Microscope has a `Datasets` collection as well as a `Comments` collection.
...
Microscope has a `Datasets` and `Comments` collection, but both aren't visible yet in the admin thingy. That's because they're not `Orion` collections yet.

Let's make them appear.

```javascript
// This is what it used to be:
// Datasets = new Mongo.Collection('posts');

// Instead let's do:
Datasets = new orion.collection('posts', {
...
Now it looks like we got `Datasets` appearing in `OrionJS`.
...
Let's start by defining a schema for our `Datasets` collection all the way at the very bottom of `/lib/collections/posts.js`. I'm too lazy to type so read the comments.
...
How about we do the same thing to the `Comments` collection as we did to the `Datasets` collection?
...
Meteor.methods({
  commentInsert: function(commentAttributes) {
    ...
    var user = Meteor.user();
    var post = Datasets.findOne(commentAttributes.postId);
    ...
    Datasets.update(comment.postId, {$inc: {commentsCount: 1}});
...
So let's use this to subscribe to the `Datasets` collection on the `comments.index` template.

```javascript
/client/templates/orion/template_subscription.js
...
Now go back to the `Comments` index page and do `Datasets.find().count()` to see that you've got `Datasets` now!
    ...
    columns: [
      { 
        ...
        render: function (val, type, doc) {
          var postId = val;
          var postTitle = Datasets.findOne(postId).title;
          return postTitle;
        }
      },
      orion.attributeColumn('createdAt', 'submitted', 'FREEDOM!!!'),
...
`W20150731-06:35:14.565(-7)? (STDERR) ReferenceError: Datasets is not defined`
...
Meteor will load `comments.js` first because, for files residing in the same folder, Meteor will load files first in numerical order and then in alphabetical order. The problem arises because the `Comments` schema defines the relationship of the `Comments` collection to the `Datasets` collection. The code in `comments.js` is referencing `Datasets`, which doesn't exist yet because `Post = new orion.collection('posts', {...})` in `Datasets.js` hasn't run yet. Basically, Meteor's trying to do this:

1. create the `Comments` collection.
2. define the `Comments` schema, which requires the `Datasets` collection to exist.
3. create the `Datasets` collection.
4. define the `Datasets` schema, which will require the `Comments` collection to exist.

So it errors out at `#2`. Well.... this is awkward. 

Ideally, we would like to do things in this order:

1. create the `Datasets` and `Comments` collections.
2. define the `Datasets` and `Comments` schemas, which depend on the above collections existing beforehand.
...
One `Post` has many `Comments`. Let's go to the schema for `Datasets`
