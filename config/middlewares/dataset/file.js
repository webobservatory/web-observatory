...
var formidable = require('formidable'),
    ...
    visRoot = path.join(__dirname, '../../../vis/');
...
exports.fileUploadzip = function (req, res) {

    var usrFileFolder;

    usrFileFolder = path.join(visRoot, req.user.email);
    ...
    form.parse(req, function (err, fields, files) {
        ...
        var old_path, orgn_fn, file_size, new_fn, new_fin, new_path,new_path_f, ext;
        old_path = files.file.path;
        ...
        file_size = files.file.size;
        new_fn = ranFN(); //foldername
        new_fin = new_fn+ext; //file name
        new_path = path.join(usrFileFolder, new_fn);
        new_path_f = path.join(usrFileFolder, new_fin);

        while (fs.existsSync(new_path)) {
            new_fn = ranFN(); //folder name
            new_path = path.join(usrFileFolder, new_fn);
        }
//console.log(ext);
        ...
        {
            //unzip
            fs.createReadStream(old_path).pipe(unzip.Extract({ path: new_path })).on("close", function(){
            if (!fs.existsSync(new_path+"/index.html"))
            {
                res.status(500);
                res.json({'success': false, error: "zip do not conain index.html"});
            }
...
//            old_path
            ...
            res.status(500);
            res.json({'success': false, error: "Not a zip"});
        }

        fs.rename(old_path, new_path_f, function (err) {
            if (err) {
                res.status(500);
                res.json({'success': false, error: err});
            } 
        ...
        var filePath = path.join(req.user.email, new_fn, "index.html");
        res.status(200);
        res.json({'success': true, path: filePath, size: file_size});
